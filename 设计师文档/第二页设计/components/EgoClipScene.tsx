...
export default function EgoClipScene() {
  ...
  useEffect(() => {
    ...
    const hemi = new THREE.HemisphereLight(0xf8fbff, 0x27364d, 2.25);
    scene.add(hemi);
    ...
    Promise.all([
      ...
      const frontColor = new THREE.Color(0xf1f3f6);
      ...
      pivot.scale.setScalar(2.45 / Math.max(size.x, size.y, size.z));
    });

    const shadow = new THREE.Mesh(
      new THREE.CircleGeometry(1.65, 64),
      new THREE.ShadowMaterial({ color: 0x506070, opacity: 0.12 }),
    );
