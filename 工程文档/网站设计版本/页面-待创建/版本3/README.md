# EgoClip landing page · version 3

> GitHub 页面版本：版本3 · 交付日期：2026-08-18 · 上一版本：`../版本2/`

这是一个可直接本地运行的 EgoClip 产品官网交付版。版本 3 只重构第 01 章：从家中入门视角看向鞋柜，钱包、钥匙和 EgoClip 随用户回家落下；点击 EgoClip 后，钱包会收到一笔不标金额的概念收入。第 02—09 章保持版本 2 的内容与交互。
...
- 首页桌面与手机居家背景由内置图像生成工具为本项目生成；钱包、钥匙、EgoClip 和收入路径均为独立 DOM 元素，以保留真实交互。
...
- 页面没有虚构续航、防水、分辨率、存储、无线连接或回报数字；首页收入反馈只表达交互意图，不承诺金额，也不处理支付。
